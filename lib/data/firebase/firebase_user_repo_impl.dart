...
class FirebaseUserRepository implements UserRepository {
    ...
    try {
      ...
      if (result.exists) {
        ...
        if (updatedUser == user) {
          ...
          return const Result.failed('Failed when update user');
        }
      } else {
        return const Result.failed('Failed when update user');
      }
    } on FirebaseException catch (e) {
      return Result.failed(e.message ?? 'Failed when update user');
    }
  }
    ...
    if (result.exists) {
      ...
      if (updateResult.exists) {
        ...
        if (updateUser.balance == balance) {
          ...
          return const Result.failed('failed get update user balance');
        }
      } else {
        return const Result.failed('failed retrive update user balance');
      }
    } else {
      return const Result.failed('user not found');
    }
  ...
  Future<Result<User>> uploadProfilePict(
    ...
    try {
      ...
      String donwloadUrl = await reference.getDownloadURL();

      var updateResult =
          await updateUSer(user: user.copyWith(photoUrl: donwloadUrl));
      ...
      return const Result.failed('failed upload image');
    }
  }
}
